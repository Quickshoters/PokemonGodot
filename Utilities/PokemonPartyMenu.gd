...
func switch_poke_order():
	...
	# Verificar si el nuevo Pokémon tiene movimientos válidos
	if Global.pokemon_group[selection].get_moves().size() == 0:
		print("Error: No valid moves for selected Pokémon.")
	else:
		print("Pokémon switched and ready for battle.")
...
# Function: check_if_battler_is_already_out
func check_if_battler_is_already_out(battler):
	return battler.is_in_battle  # Check if a battler is already in the battle
	switching = true
	swap_select = selection
	change_slot_texture(null, swap_select)
	pass
...
func get_slot_original_pos_by_selection(var sel):
	match sel:
		...
		S6:
			return original_s6
func set_prompt1(message : String):
	$Prompt/Prompt.text = message
	$Prompt/Prompt/Shadow.text = message
