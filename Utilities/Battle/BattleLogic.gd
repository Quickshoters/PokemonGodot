...
func generate_action_queue(player_command : BattleCommand, foe_command : BattleCommand):
		...
		if command.command_type == command.ATTACK:
			...
			if does_attack_hit(move, target_index, battler_index):
				...
				if move.base_power != null || move.name == "Low Kick":
					...
					# Charge bounus:
					if get_past_moves_by_index(battler_index).size() > 0 and get_past_moves_by_index(battler_index).back() == "Charge" && move.type == Type.ELECTRIC:
						other_modifer = 2.0
	...
	return queue
func get_turn_order(player_command: BattleCommand, foe_command: BattleCommand) -> void: # For single battles
	# General turn order logic:
	# 1. Item use/Running
	# 2. Switching
	# 3. Megaevolution
	...
	# If a player is trying to run, handle the escape logic
	if player_command.command_type == player_command.RUN:
		...
		if !can_escape:
			...
			action.battle_text = "Can't escape!"
			queue.push(action)
			turn_order.push_back(B2)  # Enemy gets the turn if escape fails
			return

	if player_command.command_type == player_command.USE_BAG_ITEM or player_command.command_type == player_command.SWITCH_POKE:
		turn_order.push_back(B2)  # If using an item or switching, the enemy goes next
		return

	if player_command.command_type == player_command.ATTACK and foe_command.command_type == foe_command.ATTACK:
		print("Processing turn order logic for if both commands are attacks")

		# Check if battler1 and battler2 are available
		if battler1.current_hp == 0:  # If battler1 is fainted
			print("Error: battler1 is fainted and cannot attack.")
			return  # Terminar aquí si no puede atacar

		if battler2.current_hp == 0:  # If battler2 is fainted
			print("Error: battler2 is fainted and cannot attack.")
			return  # Terminar aquí si no puede atacar
		...
		# Verifica si los movimientos son nulos
		if move_b1 == null:
			print("Error: move_b1 is null for attack move: " + player_command.attack_move)
			return  # Manejo del error aquí

		if move_b2 == null:
			print("Error: move_b2 is null for attack move: " + foe_command.attack_move)
			return  # Manejo del error aquí

		# Comparar prioridades de los movimientos
		if move_b1.priority > move_b2.priority:
			...
			turn_order.push_back(B2)
			turn_order.push_back(B1)
		else:  # Si la prioridad de los movimientos es la misma, comparar velocidad
			# Calculate effective in-battle speed
			var b1_speed = battler1.speed
			var b2_speed = battler2.speed
			b1_speed *= BattleStatStage.get_multiplier(battler1_stat_stage.speed)
			b2_speed *= BattleStatStage.get_multiplier(battler2_stat_stage.speed)
...
func percent_chance(n : float) -> bool:
	...
	if one_in_n_chance(1/n):
		return true
	return false
func post_damage_checks(battler_index: int) -> bool:
	# Check if target faints.
	var target = get_battler_by_index(battler_index)
	...
	if target.current_hp == 0:
		...
		if battler_index == 2 || battler_index == 4:
			action.battle_text = "The foe " + target.name + " fainted!"
			get_exp = true
		elif battler_index == 1 || battler_index == 3:
			action.battle_text = target.name + " fainted!"
		
		queue.push(action)

		target.major_ailment = null
		
		# If foe faint, add exp to player Pokémon
		if get_exp:
			action = BattleQueueAction.new()
			action.type = action.BATTLE_TEXT
			var exp_gained: int = calculate_exp(target)
			...
			if levelUptimes == 0: # Did not level up. Just add exp
				...
				for i in range(levelUptimes): # For each time you level up
					...
					# Apply Level Up changes to Pokémon here:
					battler1.level += 1
		...
		# Check if player or foe runs out of Pokémon
		...
		if player_defeated || foe_defeated:
			...
			if not player_defeated && foe_defeated:
				action.winner = action.PLAYER_WIN
			elif player_defeated && not foe_defeated:
				action.winner = action.FOE_WIN
			
			queue.push(action)
			return true

		# Add new Pokémon action
		action = BattleQueueAction.new()
		action.type = action.NEXT_POKE
		action.damage_target_index = battler_index
		queue.push(action)

		# Debug: Print details of the new Pokémon
		var new_battler = get_battler_by_index(battler_index)

		# Verifica si new_battler es null antes de intentar acceder a sus movimientos
		if new_battler != null:
			var moves = new_battler.get_moves()  # Asegúrate de que get_moves() devuelva los movimientos válidos
			print("New Battler Moves: ", moves)
			print("New Battler Name: ", new_battler.name)
			print("New Battler HP: ", new_battler.current_hp)
		else:
			print("Error: El nuevo Pokémon es null. No se puede acceder a sus movimientos.")
