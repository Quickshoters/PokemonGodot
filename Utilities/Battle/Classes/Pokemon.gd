...
func get_battle_foe_sprite() -> AnimatedSprite:
	var animated_sprite = AnimatedSprite.new()
	var sprite_frames : SpriteFrames

	# Determinar la ruta del recurso SpriteFrames
	var sprite_frames_path : String

	if is_shiny:
		sprite_frames_path = "res://Graphics/Battlers/" + str("%04d" % ID) + "fs.tres"
	else:
		sprite_frames_path = "res://Graphics/Battlers/" + str("%04d" % ID) + ".tres"

	# Cargar el recurso SpriteFrames
	sprite_frames = load(sprite_frames_path) as SpriteFrames

	if sprite_frames:
		# Asignar SpriteFrames al nodo AnimatedSprite
		animated_sprite.frames = sprite_frames
		animated_sprite.name = "AnimatedSprite"

		# Configurar el material del AnimatedSprite (opcional)
		var shader_material = ShaderMaterial.new()
		shader_material.shader = load("res://Utilities/Battle/StatChange.shader")
		shader_material.set_shader_param("effect_weight", 0.0)
		animated_sprite.material = shader_material

		# Configurar la animación por defecto si existe
		var default = "default"
		if sprite_frames.has_animation(default):
			animated_sprite.animation = default
		else:
			print("Advertencia: La animación '", default, "' no existe en los SpriteFrames2.")
			# Seleccionar la primera animación disponible si no se encuentra la predeterminada
			var animations = sprite_frames.get_animation_names()
			if animations.size() > 0:
				animated_sprite.animation = animations[0]
			else:
				print("Error: No hay animaciones disponibles en los SpriteFrames.")

		# Asegurarse de que la animación se reproduzca
...
func get_battle_player_sprite() -> AnimatedSprite:
	var animated_sprite = AnimatedSprite.new()
	var sprite_frames : SpriteFrames
	...
	if is_shiny:
		sprite_frames_path = "res://Graphics/Battlers/" + str("%04d" % ID) + "bs.tres"
	else:
		sprite_frames_path = "res://Graphics/Battlers/" + str("%04d" % ID) + "b.tres"
	...
	if sprite_frames:
		...
		if sprite_frames.has_animation(default):
			...
			print("Advertencia: La animación '", default, "' no existe en los SpriteFrames1.")
			# Seleccionar la primera animación disponible si no se encuentra la predeterminada
			var animations = sprite_frames.get_animation_names()
			if animations.size() > 0:
				animated_sprite.animation = animations[0]
